/* Working and deployed at home.. 7-5-2017 GRV */
...
bool show_date = false;                   // Show date? - Display date approx. every 2 minutes (default = true)
...
//  get_time(); //get the time from the clock chip
  ...
  while (run_mode()) {
    ...
    if (hours == 0) { hours = 12; }
       ...
       if      (mins >= 5 && mins <= 9)  { strcpy (str_a, "FIVE");     strcpy (str_b, "AFTER"); strcpy (str_c, ""); }
       else if (mins >= 10 && mins <= 14){ strcpy (str_a, "TEN");     strcpy (str_b, "AFTER"); strcpy (str_c, ""); }
       else if (mins >= 15 && mins <= 19){ strcpy (str_a, "QUARTER");  strcpy (str_b, "AFTER"); strcpy (str_c, ""); }
       else if (mins >= 20 && mins <= 24){ strcpy (str_a, "TEN");     strcpy (str_b, "IN FRONT");  strcpy (str_c, "HALF"); halb = 1; }      
       else if (mins >= 25 && mins <= 29){ strcpy (str_a, "FIVE");     strcpy (str_b, "IN FRONT");  strcpy (str_c, "HALF"); halb = 1; }      
       else if (mins >= 30 && mins <= 34){ strcpy (str_a, "");         strcpy (str_b,    "");  strcpy (str_c, "HALF"); halb = 1; }      
       else if (mins >= 35 && mins <= 39){ strcpy (str_a, "FIVE");     strcpy (str_b, "AFTER"); strcpy (str_c, "HALF"); halb = 1; }      
       else if (mins >= 40 && mins <= 44){ strcpy (str_a, "TEN");     strcpy (str_b, "AFTER"); strcpy (str_c, "HALF"); halb = 1; }       
       else if (mins >= 45 && mins <= 49){ strcpy (str_a, "QUARTER");  strcpy (str_b, "IN FRONT");  strcpy (str_c, ""); halb = 1; }     
       else if (mins >= 50 && mins <= 54){ strcpy (str_a, "TEN");     strcpy (str_b, "IN FRONT");  strcpy (str_c, ""); halb = 1; }       
       else if (mins >= 55 && mins <= 59){ strcpy (str_a, "FIVE");     strcpy (str_b, "IN FRONT");  strcpy (str_c, ""); halb = 1; } 
...
void get_time()
{
  ...
  rtc[5] = now.month();
  rtc[4] = now.day();
  rtc[3] = dayOfWeek(rtc[6],rtc[5],rtc[4]); //returns 0-6 where 0 = Sunday
...
void playHourBuzzer() {
    if((rtc[0] >= 2 && rtc[0] < 30 ) && rtc[1] == 0 && buzzerPlaying == 0 && rtc[2] >= 7 && rtc[2] <= 21) { // if 00 min and seconds
      buzzerPlaying = 1 ;
      playBuzzer(rtc[2]);
      buzzerPlaying = 0 ;
      return;
