...
https://gist.github.com/deanet/36c956540f240567f428
...
bool showingCalender = 0;                   // Hold buzzer playing state
...
void small_mode() {
  ...
  while (run_mode()) {
    ...
    if (buttonA.uniquePress() || checkRemoteButton() == 1 ) {
      switch_mode();
      return;
    }
    if (buttonB.uniquePress() || checkRemoteButton() == 2 ) {
      display_date();
      return;
    }
...
void display_date()
{
  ...
  byte date = rtc[4];
  byte month = rtc[5] - 1;
  //Serial.print(" DOW " );Serial.print(dow );Serial.print(" date " );Serial.print(date );Serial.print(" month " );Serial.print(month );Serial.print(" year " );Serial.println(rtc[6] );
...
void switch_mode() {
  ...
  for (int count = 0; count < 35 ; count++) {
    ...
    if (firstrun == 1 ) {
      if( checkRemoteButton() == 1  || firstrun == 1 ) {
            count = 0;
            cls();
...
void setup_menu() {
  ...
  for(int count=0; count < 35 ; count++) {
    ...
    //if user hits button, change the clock_mode
    if(buttonA.uniquePress() || firstrun == 1 || checkRemoteButton() == 1 ){
...
void showCalenderInInterval(){
  if(rtc[1] != 0 && (rtc[1]%7) == 0 && rtc[0] == 0 && showingCalender == 0 ){
    showingCalender == 1;
    display_date();
    showingCalender == 0;
    return;
...
void playHourBuzzer() {
    if(rtc[0] == 2 && rtc[1] == 0 && buzzerPlaying == 0 && rtc[2] >= 7 && rtc[2] <= 21) { // if 00 min and seconds
      buzzerPlaying = 1 ;
      playBuzzer(rtc[2]);
      buzzerPlaying = 0 ;
      return;
...
byte checkRemoteButton(){
      ...
      if (irrecv.decode(&results)) {
          ...
          else if( String(results.value, DEC) == "16220287" ) {
