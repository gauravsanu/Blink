...
https://cloud.githubusercontent.com/assets/275259/11287953/2a6e9bc6-8f52-11e5-958a-7e12e4623413.jpg
...
bool random_mode = 1;                    // Define random mode - changes the display type every few hours. Default = 0 (off)
...
bool buzzerPlaying = 0;                  // Hold buzzer playing state
bool showingCalender = 0;                // Hold calender display state
...
#define SLIDE_DELAY 30                   // The time in milliseconds for the slide effect per character in slide mode. Make this higher for a slower effect
#define cls          clear_display       // Clear display

RTC_DS1307 ds1307;                              // Create RTC object

Button buttonA = Button(7, BUTTON_PULLUP);      // Setup button A (using button library)
Button buttonB = Button(8, BUTTON_PULLUP);      // Setup button B (using button library)

void setup() {

  digitalWrite(7, HIGH);                 // turn on pullup resistor for button on pin 2
  digitalWrite(8, HIGH);                 // turn on pullup resistor for button on pin 3
...
void slide() {
  ...
  if( showingCalender == 1 ) {
    digits_old[0] = 99; digits_old[1] = 99; digits_old[2] = 99; digits_old[3] = 99;
  }
  ...
  while (run_mode()) {
    ...
    if (irrecv.decode(&results)) {
      ...
      if( String(results.value, DEC) == "16220287" ) {
          //Serial.println("B Remote button pressed under slide ");
          digits_old[0] = 99; digits_old[1] = 99; digits_old[2] = 99; digits_old[3] = 99;
          irrecv.resume(); // Receive the next value
          display_date();
      }
    ...
    if (rtc[0] != old_secs) {
      ...
      //Serial.print("New Digit");Serial.print(digits_new[3] );Serial.print(" " ); Serial.print(digits_new[2] ); Serial.print(" " ); Serial.print(digits_new[1] );Serial.print(" ");Serial.println(digits_new[0] );
      //Serial.print("Old Digit");Serial.print(digits_old[3] );Serial.print(" " ); Serial.print(digits_old[2] ); Serial.print(" " ); Serial.print(digits_old[1] );Serial.print(" ");Serial.println(digits_old[0] );
      ...
      for (byte i = 0; i <= 3; i++) {
        ...
        if (digits_old[i] != digits_new[i]) {
          ...
          for (byte seq = 0; seq <= 8 ; seq++) {
            ...
            Serial.print(old_char[0] ); Serial.print(" => New " ); Serial.println(new_char[0] );
            //delay(100);
            slideanim(digits_x_pos[i], 0, seq, old_char[0], new_char[0]);
            delay(SLIDE_DELAY);
          }
          //delay(100);
          Serial.print(digits_old[i] ); Serial.print(" => New " ); Serial.println(digits_new[i] );
        }
      }
...
void word_clock() {
  ...
  while (run_mode()) {
    ...
    if (irrecv.decode(&results)) {
      if( String(results.value, DEC) == "16187647"  ) {
        Serial.println("A"); //first button pressed under world clock ");
        switch_mode();
        return;
      }

      if( String(results.value, DEC) == "16220287" ) {
          Serial.println("B");// First button pressed under world clock ");
          display_date();
      }
      irrecv.resume(); // Receive the next value
    }
    ...
    while (counter > 0){
      ...
      if (irrecv.decode(&results)) {
        if( String(results.value, DEC) == "16187647"  ) {
          //Serial.println("A second button pressed under world clock ");
          switch_mode();
          return;
        }
        
        if( String(results.value, DEC) == "16220287" ) {
            //Serial.println("B second button pressed under world clock ");
            display_date();
        }
        irrecv.resume(); // Receive the next value
      }
    ...
    while (counter > 0){
      ...
      if (irrecv.decode(&results)) {
        if( String(results.value, DEC) == "16187647"  ) {
          //Serial.println("A third button pressed under world clock ");
          switch_mode();
          return;
        }
        
        if( String(results.value, DEC) == "16220287" ) {
            //Serial.println("B third button pressed under world clock ");
            display_date();
        }
        irrecv.resume(); // Receive the next value
      }
    ...
    while (counter > 0){
      ...
      if (buttonB.uniquePress()) {
        ...
        if (irrecv.decode(&results)) {
        if( String(results.value, DEC) == "16187647"  ) {
          //Serial.println("A fourth button pressed under world clock ");
          switch_mode();
          return;
        }
        
        if( String(results.value, DEC) == "16220287" ) {
            //Serial.println("B fourth button pressed under world clock ");
            display_date();
        }
    ...
    if (irrecv.decode(&results)) {
      if( String(results.value, DEC) == "16187647"  ) {
        //Serial.println("A fifth button pressed under world clock ");
        switch_mode();
        return;
      }
      
      if( String(results.value, DEC) == "16220287" ) {
          //Serial.println("B fifth button pressed under world clock ");
          display_date();
      }
...
void switch_mode() {
  ...
  for (int count = 0; count < 35 ; count++) {
    ...
    if (irrecv.decode(&results) || firstrun == 1 ) {
      if( String(results.value, DEC) == "16187647" || firstrun == 1 ) {
            //Serial.println("A Remote button pressed undeer switch mode");
            results.value = 0 ;
            count = 0;
            cls();
...
void setup_menu() {
  ...
  for(int count=0; count < 35 ; count++) {
    ...
    if (irrecv.decode(&results) || firstrun == 1 ) {
      if( String(results.value, DEC) == "16187647" || firstrun == 1 ) {
            //Serial.println("A Remote button pressed undeer setup mode");
            results.value = 0 ;
            count = 0;
            cls();
...
if(irrecv.decode(&results)) {
    while(String(results.value, DEC) != "16187647") {
      Serial.println("A button not yet pressed under set_time mode ");
        //irrecv.resume(); // Receive the next value
      while (String(results.value, DEC) == "16220287"){
        //Serial.println("B button pressed under set_time mode ");
        //irrecv.resume(); // Receive the next value
